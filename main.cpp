...
size_t decode(uint8_t* buffer, uint64_t* converted, uint32_t coded_data_size, uint32_t data_size)
{
    size_t potential_ints_lsb = 0;
    size_t potential_ints_msb = 0;

    size_t boffset = 0;
    while (boffset < coded_data_size) {
        potential_ints_msb++;
    ...
    size_t bytes_used = 0;
    size_t int_count = 0;
    while (bytes_used < coded_data_size) {
        if (int_count >= data_size/4) {
...
int main(int argc, char** argv)
{
    ...
    while ((file_length - f.tellg()) >= sizeof(philips::label)) {
        ...
        if (l.new_.label_type > philips::LABEL_TYPE_MIN && l.new_.label_type < philips::LABEL_TYPE_MAX) {
            ...
            uint32_t int_count = decode(buffer, converted, l.new_.coded_data_size, l.new_.data_size);
            std::cout << "Ints expected: " << l.new_.data_size/4 <<
                    ", ints decoded: " << int_count << std::endl;
